...
class FilterDrinkIngredientDialogFragment :
    ...
    override fun onListItemClick(v: View?) {
        ...
        if (selectedElements.contains(
                IngredientModel(DrinkFilterType.INGREDIENT, DRINK_FILTER_ABSENT)
            ...
            val noneElement: AppCompatButton? =
                extraContentView?.findViewWithTag(IngredientModel(DrinkFilterType.INGREDIENT, DRINK_FILTER_ABSENT))
            noneElement?.isSelected = false
        }
        if (selectedElements.isEmpty()) {
            selectedElements.add(IngredientModel(DrinkFilterType.INGREDIENT, DRINK_FILTER_ABSENT))
            val noneElement: AppCompatButton? =
                extraContentView?.findViewWithTag(IngredientModel(DrinkFilterType.INGREDIENT, DRINK_FILTER_ABSENT))
