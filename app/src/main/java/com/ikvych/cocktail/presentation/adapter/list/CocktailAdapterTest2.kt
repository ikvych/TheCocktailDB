...
class CocktailAdapterTest2(
            ...
            return when (completedDataList[position].second) {
                ViewType.HEADER -> 4
                    ...
                    if (isFavorite) {
                        ...
                        if (currentHeader != null) {
                            val rest = typedDataMap[currentHeader]!!.size % 4
                            if (rest == 0) {
                                if (orientation == Configuration.ORIENTATION_LANDSCAPE || sortType == SortDrinkType.INGREDIENT_COUNT_ASC || sortType == SortDrinkType.INGREDIENT_COUNT_DESC ) {
                                    1
                                } else {
                                    2
                                }
                            } else if (rest == 3 || rest == 1) {
                                val index =
    ...
    private fun determineSpanDependingOnOrientation(): Int {
        return if (orientation == Configuration.ORIENTATION_LANDSCAPE) 1 else 2
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        if (itemMaxHeight == null) {
            ...
            val num = if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
                4
            } else {
                2
            }
            val layoutWidth = layoutManager.width / 2
